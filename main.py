...
class tools():
		...
		while True:
			...
			for i in range(min(len(monsters) - t, 2)):
				if i + t < len(monsters):
					...
					this_scenes.append(text_object(self.screen, font.render("HP: " + str(monsters[t + i][2]) , True , (255,255,255)), (3.5, 5.2 * i + 1)))
					this_scenes.append(text_object(self.screen, font.render("AS: " + str(monsters[t + i][3]) , True , (255,255,255)), (6, 5.2 * i + 1)))
					this_scenes.append(text_object(self.screen, font.render("DS: " + str(monsters[t + i][4]) , True , (255,255,255)), (8.5, 5.2 * i + 1)))
					this_scenes.append(text_object(self.screen, font.render("Money: " + str(monsters[t + i][5]) , True , (255,255,255)), (3.5, 5.2 * i + 1.75)))
					this_scenes.append(text_object(self.screen, font.render("Estimated D: " + str(damage), True , (255,255,255)), (8.5, 5.2 * i + 1.75)))
					if len(monsters[t + i][-1]) >= 13:
						this_scenes.append(text_object(self.screen, font.render("Intro: ", True , (255,255,255)), (3.5, 5.2 * i + 2.5)))
						...
						this_scenes.append(text_object(self.screen, font.render("Intro: " + monsters[t + i][-1], True , (255,255,255)), (3.5, 5.2 * i + 2.5)))

			this_scenes.append(text_object(self.screen, font.render("(Q（or D） Exit； arrow: switching pages)", True , (255,255,255)), (5.8, 11.2)))
	...
	def fly(self):
		...
		while True:
			key = conversation_control.print_word("","You wanna Go to " + str(t[m]) + " level",keys = [pygame.K_UP,pygame.K_DOWN, ord('w'), ord('s'),ord('q'),ord("f"), 32],prompt = "（Q Cancel）")
			...
			if key == ord('w') or key == pygame.K_UP:
				...
				if t[m] >= parameter['this_floor']:
					if "from_lower" in floors[t[m]].config:
						...
						conversation_control.print_word("","Unable to land")
				else:
...
class fight():
	...
	def fight_with(self, monster):
		...
		this_scenes.append(text_object(self.screen, font.render(str("Warrior") , True , (255,255,255)), (9.5, 1.3)))
...
class conversation():
	...
	def choice(self, path, text, choices,prompt = "", now_index = 0):
		...
		while True:
			...
			if prompt:
				self.objects.append(text_object(self.screen, font.render("（UP/DOWN to select、SPACE to buy、Q to cancel）" , True , (255,255,255)), (8, 6.7)))
			else:
	...
	def print_word(self, name, text, path = "",prompt = "", keys = []):
		...
		if prompt == "":
			if keys == []:
				self.objects.append(text_object(self.screen, font.render("Press To Exit" , True , (255,255,255)), (11, 11.5)))
			else:
...
"""
# English translate, but not working with game right now
scenes.append(text_object(self.screen, font.render("Floor: ", True, (255,255,255)), (-4, -0.25)))
scenes.append(text_object(self.screen, font.render("HP: ", True, (255,255,255)), (-5.5, 0.75)))
scenes.append(text_object(self.screen, font.render("Attack: ", True, (255,255,255)), (-5.5, 1.75)))
scenes.append(text_object(self.screen, font.render("Defense: ", True, (255,255,255)), (-5.5, 2.75)))
scenes.append(text_object(self.screen, font.render("Level: ", True, (255,255,255)), (-5.5, 3.75)))
"""
